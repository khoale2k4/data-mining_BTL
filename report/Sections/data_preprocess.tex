...
\begin{itemize}
    ...
    \item \textbf{Biện luận:} Để tập trung vào dự báo nhu cầu mua sắm thay vì dòng tiền kế toán, quyết định đưa các giá trị âm về 0 để mô hình không bị nhiễu bởi các giao dịch hoàn trả phức tạp.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.8\linewidth]{Images/sales_seasonality.png}
        \caption{Tính thời vụ}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{Images/sales_vs_size_scatter.png}
    \caption{Mối quan hệ giữa Weekly\_Sales và Size}
    \label{fig:sales_vs_size_scatter}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{Images/sales_by_type_boxplot.png}
    \caption{Mối quan hệ giữa Weekly\_Sales và các Type}
    \label{fig:sales_by_type_boxplot}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{Images/feature_importance.png}
    \caption{Tầm quan trọng của đặc trưng}
...
Để đảm bảo tính khách quan và tránh hiện tượng Rò rỉ dữ liệu (Data Leakage), việc chuẩn hóa sẽ không được thực hiện ngay trên toàn bộ tập dữ liệu gốc. Thay vào đó, quy trình sẽ là:
\begin{itemize}
    \item Giữ nguyên dữ liệu thô ở bước này.
    
    \item Khởi tạo đối tượng Scaler.
    
    \item Việc tính toán tham số (fit) và biến đổi (transform) sẽ được thực hiện sau khi dữ liệu đã được chia tách thành tập Huấn luyện (Train) và Kiểm tra (Test).
\end{itemize}

\subsection{Kết quả sau tiền xử lý}

\subsubsection{Thay đổi về cấu trúc và tính đầy đủ}
...
\begin{itemize}
    ...
    \item \textbf{Xử lý giá trị thiếu:} Tất cả 21 cột hiện tại đều có 421{,}570 giá trị \texttt{non-null}. Điều này xác nhận rằng 5 cột \texttt{MarkDown1-5} đã được điền đầy đủ bằng giá trị 0.
...
Tập dữ liệu \texttt{df} hiện đã hoàn toàn sạch (không còn giá trị thiếu hay nhiễu) và có cấu trúc phù hợp (đã trích xuất đặc trưng số). Nó đã sẵn sàng cho bước tiếp theo là \textbf{phân chia dữ liệu (train/test split)} và đưa vào các mô hình học máy để huấn luyện.
